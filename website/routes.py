  ...
  if request.method == "POST":
    ...
    if p_item_object:
      if current_user.can_purchase(p_item_object, quantity_bought):
       ...
       flash(f"You purchased { p_item_object.name } for ₹{ quantity_bought*p_item_object.price }", category="success")
      else:
        flash(f"Error while purchasing {p_item_object.name}", category="danger")
    #sell
    ...
    if s_item_object:
      ...
      flash(f"You sold { s_item_object.name } for ₹{ quantity_bought*s_item_object.price }", category="success")
...
def login_page():
 ...
 if form.validate_on_submit():
  user_to_create = User(username=form.username.data, email_address=form.email_address.data, phone_number = form.phone_number.data, password=form.password1.data, address = form.address.data)
  ...
  if request.method == "POST":
    ...
    if p_fert_object:
      if current_user.can_purchase(p_fert_object, quantity_bought_f):
       ...
       flash(f"You purchased { p_fert_object.name } for ₹{ quantity_bought_f*p_fert_object.price }", category="success")
      else:
        flash(f"Not enough cash in account to purchase {p_fert_object.name}", category="danger")
